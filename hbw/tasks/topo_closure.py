...
class TopoEmulationClosure(
    ...
    def _summary_text(self, integrated, differential, datasets, variables) -> str:
        ...
        lines.append("and are credited with nothing. READ THE TWO TOGETHER, and read them knowing")
        lines.append("the following, because it is the single easiest thing to get wrong here.")
        lines.append("")
        lines.append("The support cut is a two-b-tag requirement, and it plays TWO roles that are")
        lines.append("the same selection but not the same requirement:")
        lines.append("  (1) purity of the CONTROL SAMPLE the estimator is fitted on. Legitimate,")
        lines.append("      and it does real work -- it is what makes the measurement sample")
        lines.append("      top-dominated rather than QCD-dominated.")
        lines.append("  (2) support of the DEPLOYED estimator, i.e. where emulation may apply at")
        lines.append("      all. This is inherited from (1) only as a side effect, because a model")
        lines.append("      cannot be evaluated where it was never fitted.")
        lines.append("")
        lines.append("The justification is entirely (1); the blend bias below is entirely (2).")
        lines.append("Conflating them is the defect, not the cut. It matters because the trigger")
        lines.append("itself is NOT b-tag gated -- its requirements are an L1 seed, a 12 GeV muon")
        lines.append("leg, online HT > 50 GeV and a discriminant score -- so an out-of-support")
        lines.append("event CAN fire it, and crediting it with nothing under-counts the trigger by")
        lines.append("more on b-poor samples than on b-rich ones. Signal is b-rich and QCD is not,")
        lines.append("so the asymmetry flatters any S/sqrt(B) built on these weights, and the gap")
        lines.append("between the two gain columns is the size of the flattery.")
        lines.append("")
        lines.append("DO NOT read that as an argument for widening the support. The same cut that")
        lines.append("purifies the control sample is the cut that removes QCD, which is precisely")
        lines.append("the process whose trigger response is least known -- so a wider support")
        lines.append("trades a modelling problem that is under control for one that is not. The")
        lines.append("tension is intrinsic. Reporting both columns brackets it honestly without")
        lines.append("having to resolve it.")
        lines.append("")
