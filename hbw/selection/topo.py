...
``n_btag`` is deliberately left at hbw's default rather than raised to 2 to reproduce the study's
">= 2 b-tags" phase space: hbw's ``n_btag`` counts ``btagUParTAK4B`` at the medium working point for
``year >= 2024``, while the study's counting uses ``btagPNetB > 0.2``. That counting is reproduced by
