...
Summer24 NanoAODv15 and has to be emulated. The emulation consumes the *offline* feature vector the
trigger-efficiency estimator of AN-2026/124 was fit on, defined in
``topo-trigeff-cmsdata/src/extract.py``'s ``build()`` and ``topo-trigeff-cmsdata/src/config.py``.
...
offline HT is a proxy for -- the mismatch is the subject of the note's ``sec:l1ht`` and is not a
bug here.
"""
...
@producer(
    ...
    # configurable attributes, defaulting to topo-trigeff-cmsdata/src/config.py
    jet_pt=25.0,
    jet_eta=2.5,
    jet_id=False,
    mu_pt=10.0,
...
def topo_features(self: Producer, events: ak.Array, **kwargs) -> ak.Array:
    ...
    # --- L1 HT staircase proxy (7th feature; see the module docstring and the note's sec:l1ht) ---
...
#: available as *truth* and does not have to be emulated at all. This matters: the note's Appendix-A
#: test found Mu12_PFHT150_PNetB STRUCTURED in all five offline b-tag slices, i.e. it is precisely
#: the leg that must not be reweighted from offline-only variables. Only the TOPO residual is.
...
def topo_or2_weights(self: Producer, events: ak.Array, **kwargs) -> ak.Array:
    ...
    MET-orthogonal reference rather than transported from the note; until that measurement exists it
